using Ovningar20220901.IceCream;
using Ovningar20220901.SubFunctions;
...
public abstract class Exercises
{
    ...
    public static void Twenty(bool shouldShowRandom)
    {
        ...
        {
            Console.WriteLine("Vilken höjd vill du ha?");
            if (int.TryParse(Console.ReadLine(), out snake.Height))
            {
                break;
            }
        }
        ...
        {
            Console.WriteLine("Vilken bredd vill du ha?");
            if (int.TryParse(Console.ReadLine(), out snake.Width))
            {
                break;
            }
        }
        if (shouldShowRandom)
        {
            while (true)
            {
                ...
                if (int.TryParse(Console.ReadLine(), out snake.RandomShapes))
                {
                    break;
                }
            }
        ...
        {
            snake.RandomShapes = 0;
        }

        snake.DrawArray = new string[snake.Height, 1];
        var sX = snake.Width / 2;
        var sY = snake.Height / 2;
        snake.PlayerPos = new[,] { { sX, sY }, { sX, sY - 1 }, { sX, sY - 2 } }; //X, Y
    ...
    {
        Console.WriteLine(PrivateObjekt.ReturnThis());
    }

    public static void ThirtySix()
    {
        var piggelin = new Piggelin
        {
            Cost = 10
        };
        var calippo = new Calippo
        {
            Cost = 20
        };
        var listIce = new List<IceCreams>
        {
            piggelin,
            calippo
        };
        foreach (var iceCreams in listIce)
        {
            Console.WriteLine(iceCreams.Buy());
            if (iceCreams is Calippo iCalippo)
            {
                Console.WriteLine(iCalippo.BuyFive());
            }
            if (iceCreams is Piggelin iPiggelin)
            {
                Console.WriteLine(iPiggelin.BuyTen());
            }
        }
    }
    ...
    public static void ThirtySeven()
    {
        ...
        while (true)
        {
            Console.WriteLine("Skriv in något (avsluta med ENTER):");
            var userInput = Console.ReadLine();
            if (string.IsNullOrEmpty(userInput))
            {
                break;
        ...
        {
            Console.WriteLine(userStack.Pop());
        }
    }

    public static void ThirtyEight()
    {
        var personer = Person.TemplatesPersons();
        Person.SearchPerson(ref personer);
    }
    ...
    public static void ThirtyNine()
    {
        ...
        while (true)
        {
            Console.WriteLine("Skriv en sträng eller siffra");
            var userInput = Console.ReadLine();
            if (!string.IsNullOrEmpty(userInput))
            {
                if (int.TryParse(userInput, out var userResult))
                {
                    if (listString.Count > 0 && userResult > 0)
                    {
                        if (listString.Count < userResult)
                        {
    ...
    public static void Fourty()
    {
        ...
        while (true)
        {
            Console.WriteLine($"Välkommen till C#-Verket\nTryck 1 för att lägga till en person.\nTryck 2 för att söka efter en person.\nTryck 3 för att avsluta.");
            var userInput = Console.ReadLine();
            if (!string.IsNullOrEmpty(userInput))
            {
                if (int.TryParse(userInput, out var userResult))
                {
                    if (userResult is > 0 and < 4)
                    {
                        if (userResult == 1)
                        {
                            Person.AddPerson(ref personer);
                        }
